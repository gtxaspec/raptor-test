...
| Content | Measured video fps vs nominal (within 15%), resolution sanity, decoded audio sample count vs declared rate (catches SBR/rate mislabels), audio channels declared (ASC `channelConfiguration`) vs decoded — aware that ffmpeg presumes Parametric Stereo on any mono+SBR stream and shows it as 2ch ("Treating HE-AAC mono as stereo"), so that case passes only if L and R decode bit-identical; distinct content fails as real stereo leakage — GOP cadence from a raw bitstream census, per-frame SEI presence (raptor ST 0604; skipped when absent) |
